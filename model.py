...
def portfolio_simulation(tikers, iterations=3000):
    df_tikers = pd.DataFrame()
    for tiker in tikers:
        df_tikers = df_tikers.add(
            sr_year_profit(pd.read_sql_query(f"SELECT * FROM history_tiker_month WHERE secid = '{tiker}'", con=engine),
                           tiker), fill_value=0)
